...
__forceinline__ __device__ void copy(half* input, half* output, const uint64_t nelem,
    ...
    const uint64_t nFirstElem = (reinterpret_cast<uintptr_t>(input_ptr) - input_val) / sizeof(half);
    if (threadId < nFirstElem && threadId < nelem) {
        output[threadId] = input[threadId];
    }

    const uint64_t nelem4 = (nelem - nFirstElem) / (sizeof(int4) / sizeof(half));
    for (uint64_t i = threadId; i < nelem4; i += numThreads) {
        output_ptr[i] = input_ptr[i];
    }

    const uint64_t nLastElem = (nelem - nFirstElem) % (sizeof(int4) / sizeof(half));
...
extern "C" __forceinline__ __device__ void
    ...
    if (peer < nchannels) {
        sm_channels[peer].put(local_offset * sizeof(half), nelem_per_shard * sizeof(half),
                              tid + peer_block_idx * blockDim.x, n_parallel_sm_blocks * blockDim.x);
...
extern "C" __forceinline__ __device__ void
    ...
    if (threadId < nFirstElem && threadId < nelem_per_shard) {
        ...
        for (int i = 0; i < nranks - 1; ++i) {
            tmp += sm_input_buff_channels[(rank + i) % (nranks - 1)].read<half>(offset);
        }
        output[offset] = tmp;
    }
    ...
    for (uint64_t offset = threadId; offset < nelem4; offset += gridDim.x * blockDim.x) {
        ...
        for (int i = 0; i < nranks - 1; ++i) {
            ...
            *reinterpret_cast<half2*>(&tmp.x) += *reinterpret_cast<half2*>(&val.x);
            *reinterpret_cast<half2*>(&tmp.y) += *reinterpret_cast<half2*>(&val.y);
            *reinterpret_cast<half2*>(&tmp.z) += *reinterpret_cast<half2*>(&val.z);
            *reinterpret_cast<half2*>(&tmp.w) += *reinterpret_cast<half2*>(&val.w);
        }
        output4[offset] = tmp;
    }
...
//             accum[0] +=  half2float(half_ptr[0]);
//             accum[1] +=  half2float(half_ptr[1]);
//             accum[2] +=  half2float(half_ptr[2]);
//             accum[3] +=  half2float(half_ptr[3]);
//             accum[4] +=  half2float(half_ptr[4]);
//             accum[5] +=  half2float(half_ptr[5]);
//             accum[6] +=  half2float(half_ptr[6]);
//             accum[7] +=  half2float(half_ptr[7]);
//         }
...
//     if (threadId < nLastElem) {
//         const uint64_t offset = local_offset + nelem_per_shard - nLastElem + threadId;
//         float tmp = half2float(*reinterpret_cast<half*> (&input[offset]));
//         for (int i = 0; i < nranks - 1; ++i) {
//             tmp += sm_input_buff_channels[(rank + i) % (nranks - 1)].read<half>(offset);
//         }
//         output[offset] = tmp;
...
extern "C" __global__ void __launch_bounds__(1024) 
    allreduceKernelWithLNEntryPoint(mscclpp::SmChannelDeviceHandle* sm_input_buff_channels,
                                    ...
                                    const int nchannels, const uint64_t local_offset,
                                    const int rank, const int nranks, const uint64_t nelem_per_shard,
                                    half* input, half* output, half* output_before_ln,
...
extern "C" __global__ void __launch_bounds__(1024)
    ...
    if (threadId < nchannels) {
        ...
        proxy_channels[threadId].putWithSignal(local_offset * sizeof(half), nelem_per_shard * sizeof(half));
    }
}
...
extern "C" __global__ void __launch_bounds__(1024)
    ...
    if (threadId < nranks - 1) {
        ...
        proxy_channels[threadId].putWithSignal(
            get_scatch_start(rank, nelem_per_shard, nFirstElem) * sizeof(half),
            remoteRank * nelem_per_shard * sizeof(half),
            nelem_per_shard * sizeof(half));
    }
}
...
extern "C" __global__ void __launch_bounds__(1024)
    ...
    const uint64_t nFirstElem = local_offset4 * n_half_per_int4 - local_offset;
    if (threadId < nFirstElem && threadId < nelem_per_shard) {
        half tmp = input[local_offset + threadId];
    ...
    if (threadId < nLastElem) {
        ...
        half tmp = input[local_offset + offset];
        for (int r = 0; r < nranks; ++r) {
            if (r == rank) continue;
