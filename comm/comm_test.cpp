...
void test_NetRingAllGather(std::shared_ptr<mscclpp::Communicator> comm,
                       std::vector<std::shared_ptr<mscclpp::Connection>>& connections,
                       const int rank, const int nranks,
                       const size_t buff_size, const bool inplace, const bool columnwise) {
    // Intialize host and device buffers
    ...
    if (columnwise) {
        ...
        for (size_t i = 0; i < host_buff.size(); ++i) {
            ...
            __fp16 expected = __fp16(int((i * remoteRank) % 101));
            if (host_buff[i] != expected) {
                std::cerr << "Rank " << rank << " received incorrect data from rank " << remoteRank << " at index " << i << " with value " << host_buff[i] << " and expected " << expected << std::endl;
                break;
            }
        }
    }
...
void test_NetRingReduceScatter(std::shared_ptr<mscclpp::Communicator> comm,
                           std::vector<std::shared_ptr<mscclpp::Connection>>& connections,
                           const int rank, const int nranks,
                           const size_t buff_size, const bool inplace) {
    // Intialize host and device buffers
    ...
    // Initialize NetWrapper
    NetRingReduceScatter wrapper;
    int rings_topo[40] = {0, 1, 2, 3, 4, 5, 6, 7, 0, 7, 6, 5, 4, 3, 2, 1, 0, 3, 6, 1, 4, 7, 2, 5, 0, 5, 2, 7, 4, 1, 6, 3, 0, 1, 2, 3, 4, 5, 6, 7};
    wrapper.init(
        comm, connections, rank, nranks, 
        pllmTensor<half>{
                    (half*)input_buff, (int)(buff_size / sizeof(__fp16)), (int)1, PllmLayout::ROW_MAJOR},
				 pllmTensor<half>{
                    (half*)output_buff, (int)(buff_size / sizeof(__fp16)), (int)1, PllmLayout::ROW_MAJOR},
        5, rings_topo);
    ...
    for (size_t i = rank * nelem_per_shard; i < (rank + 1) * nelem_per_shard; ++i) {
        __fp16 expected = __fp16(0);
        for (int j = 0; j < nranks; ++j) expected += __fp16(int((i * j) % 101));
        if (host_buff[i] != expected) {std::cerr << "Rank " << rank << " incorrect data at index " << i << " with value " << host_buff[i] << " and expected " << expected << std::endl;
            break;
        }
    }
    ...
    CUDA_CHECK(cudaFree(input_buff));
    if (input_buff != output_buff) CUDA_CHECK(cudaFree(output_buff));
    std::cout << "Rank " << rank << " NetRingReduceScatter test ("
              << "buff_size=" << buff_size << ",inplace=" << inplace
              << ") finished" << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    test_NetRingAllGather(comm, connections, rank, nranks, buff_size, true, false);
    test_NetRingAllGather(comm, connections, rank, nranks, buff_size, false, false);
    test_NetRingReduceScatter(comm, connections, rank, nranks, buff_size, true);
    test_NetRingReduceScatter(comm, connections, rank, nranks, buff_size, false);
